...
async function macJoinCar(ssid, carIp = CAR_IP_DEFAULT, opts = {}) {
  ...
  // networksetup fails outright when the AP is not associable YET — exactly the
  // case a second or two after powering the car on, or right after the radio wakes.
  // A single shot then reports "not in range" and the user just reruns run.sh,
  // which works only because the AP came up in the meantime. Retry briefly instead.
  // opts.tries lets the caller probe a candidate ONCE (presence test across several
  // saved cars) instead of spending the full retry budget on a car that isn't there.
  const tries = Math.max(1, Number(opts.tries ?? process.env.WIFI_JOIN_TRIES ?? 4));
  const retryMs = Number(process.env.WIFI_JOIN_RETRY_MS ?? 1500); // ~4.5s of retrying
  let r;
  for (let attempt = 1; ; attempt++) {
    r = await run("networksetup", ["-setairportnetwork", iface, s, pw]); // pw as argv
    if (r.code === 0 || attempt >= tries) break;
    await sleep(retryMs);
  }
